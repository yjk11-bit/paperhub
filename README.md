...
| 元数据爬虫 | requests + BeautifulSoup 采集标题/学科/年份/来源链接/网页摘要；多频道顺序采集（高考题库/数学/语文/英语/理综/文综），逐频道容错（单频道失败记录日志并继续），robots 逐路径检查、限速、按 source_url 去重、可重复执行 |
| 后台爬虫任务 | 管理员在审核面板一键【启动爬虫】：后台线程执行采集，前端轮询展示每频道状态/解析/新增明细；同一时间只允许一个任务，重复触发友好提示；采集数据仅入待审核表，绝不自动上线 |
...
- **爬虫放弃翻页采集**：源站翻页子域 403 反爬且列表页无翻页链接，改为固定采集 6 个频道首页（高考题库/数学/语文/英语/理综/文综），按 source_url 去重、可重复执行；增量数据可依靠 CSV 导入补充。
